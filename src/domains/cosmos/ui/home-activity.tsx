...
function EmptyTransactionState() {
  ...
  return (
    <div className="flex h-full min-h-[8rem] flex-col items-center justify-center px-6 py-8 text-center">
      ...
      <p className="mt-1 max-w-72 text-sm text-slate-500">{messages.common.cosmosProviderNoTransactions}</p>
    </div>
  );
}
        ...
        <CardContent className="p-5">
          ...
          >
            {pushedBlockItems.length ? (
                    ...
                    <div className="home-activity-push-row-content">
                      <div className="grid grid-cols-[40px_minmax(110px,0.75fr)_minmax(0,1fr)_120px] items-center gap-4 py-4">
                        ...
                        <div className="truncate rounded-lg border border-slate-200 bg-slate-50 px-3 py-1 text-right text-xs text-slate-600">
                          {translateRuntimeText(block.blockSizeLabel, locale)}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <EmptyState title={LATEST_BLOCKS_TITLE} message={errorMessage ? translateRuntimeText(errorMessage, locale) : messages.common.addCosmosProviderForBlocks} />
            )}
          </div>
        </CardContent>
      </Card>
                    ...
                    <div className="home-activity-push-row-content">
                      <div className="grid grid-cols-[40px_minmax(190px,1.15fr)_minmax(0,1fr)_130px] items-center gap-4 py-4">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 truncate text-sm text-slate-500">
                            {messages.common.height} {transaction.height} · {formatRelativeAge(transaction.timestampMs, nowMs, locale)}
                          </p>
                        </div>
                        <div className="min-w-0">
                        ...
                        <div className="truncate rounded-lg border border-slate-200 bg-slate-50 px-3 py-1 text-right text-xs text-slate-600">
                          {translateRuntimeText(transaction.feeLabel, locale)}
                        </div>
                      </div>
                    </div>
                  </div>
