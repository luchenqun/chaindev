            ...
            {snapshot?.activity.blocks.length ? (
              snapshot.activity.blocks.map((block, index) => (
                <div
                  ...
                  className={`grid grid-cols-[auto_120px_minmax(0,1fr)_auto] items-center gap-4 py-4 ${index ? 'border-t border-slate-200' : ''}`}
                >
            ...
            {snapshot?.activity.transactions.length ? (
              snapshot.activity.transactions.map((transaction, index) => (
                <div
                  ...
                  className={`grid grid-cols-[auto_145px_minmax(0,1fr)_auto] items-center gap-3 py-4 ${index ? 'border-t border-slate-200' : ''}`}
                >
