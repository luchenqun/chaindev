...
function PendingTransactionsPanelSkeleton({
  className = '',
}: {
  className?: string;
}) {
  return (
    <section
      className={`overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.06)] ${className}`.trim()}
    >
...
export function PendingTransactionsPanel({
  ...
  if (!data) {
    return (
      <section
        className={`rounded-3xl border border-slate-200 bg-white px-5 py-10 shadow-[0_6px_18px_rgba(15,23,42,0.06)] ${className}`.trim()}
      >
