...
function buildFallbackItem(reason: string): StatusItem {
  return { label: "Latest Block", value: reason, toneClassName: "text-slate-500" };
}

export function ChainStatusStrip({ mode }: { mode: PlatformMode }) {
  const { status, pollIntervalMs } = useEvmHomeData();
  const [item, setItem] = useState<StatusItem>(() => buildFallbackItem("Unavailable"));
  ...
  useEffect(() => {
    ...
    if (mode === "evm") {
      setItem(
        status
          ? { label: "Latest Block", value: status.latestBlock, toneClassName: "text-sky-600" }
    ...
    async function load() {
      try {
        if (mode === "cosmos") {
          ...
          setItem({ label: "Latest Block", value: overview.latestHeight, toneClassName: "text-sky-600" });
      ...
      <span className="inline-flex items-start gap-2">
        ...
        <span className="flex flex-col leading-tight">
          <span className="inline-flex items-center gap-1.5">
            ...
            <strong className={item.toneClassName ?? "text-slate-800"}>{item.value}</strong>
          </span>
          {mode === "evm" ? (
            <span className="mt-0.5 inline-flex items-center gap-1.5">
              <span>Poll:</span>
              <strong className={item.toneClassName ?? "text-slate-800"}>{Math.round(pollIntervalMs)} ms</strong>
            </span>
          ) : null}
        </span>
      </span>
