...
import {
  clearServerCosmosAddressTags,
  deleteServerCosmosAddressTag,
  listServerCosmosAddressTags,
  upsertServerCosmosAddressTag,
} from '@/server/repositories/cosmos-address-tags';
...
export async function GET() {
  try {
    ...
    return ok(await listServerCosmosAddressTags(userId));
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return ok(
      await upsertServerCosmosAddressTag({
        userId,
        ...body,
...
export async function DELETE(request: Request) {
  try {
    ...
    if (clear === '1') {
      return ok(await clearServerCosmosAddressTags(userId, providerProfileId));
    }
    ...
    return ok(await deleteServerCosmosAddressTag({ userId, providerProfileId, address }));
  } catch (error) {
    return fail(normalizeApiError(error));
  }
}
