...
import {
  buildPageHref,
  parsePageParam,
} from '@/domains/cosmos/ui/page-query';
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.06)]">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Showing governance proposals returned by the active Cosmos
                  REST provider.
                </p>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead>
                <tr>
                  <th className="w-full max-w-[360px] border-b border-slate-200 px-4 py-3 text-left text-[13px] font-semibold text-slate-800">
                    Proposal
                  </th>
              ...
              <tbody>
                {data.proposals.length ? (
                  data.proposals.map((proposal) => (
                    <tr
                      key={proposal.id}
                      className="cursor-pointer border-t border-slate-200 hover:bg-slate-50/70"
                      onClick={() => router.push(`/cosmos/proposals/${proposal.id}`)}
                    >
                      <td
                        className="w-full max-w-[360px] px-4 py-3 text-sm"
                        title={`#${proposal.id}. ${proposal.title}`}
                      >
                        <Link
                          className="inline-block max-w-[360px] truncate align-middle font-medium text-sky-600 hover:text-sky-700"
                          href={`/cosmos/proposals/${proposal.id}`}
                        >
                      ...
                      <td
                        ...
                        title={proposal.tallyLabel}
                      >
                        <div className="min-w-[280px] truncate">
                          {proposal.tallyLabel}
                        </div>
                      </td>
